...
if __name__ == "__main__":
    ...
    def compute_metric(s, hemis=["lh", "rh"], spacing="ico4"):
        fwd_fname = save_dir + "bem/%s-%s-fwd.fif" % (s, spacing)
        if s != "fsaverage":
            src = mne.read_forward_solution(fwd_fname)["src"]
        else:
            src_fname = save_dir + "bem/%s-%s-morphed-src.fif" % (s, spacing)
        ...
        for i, h in enumerate(hemis):
            ...
            D_fname = metrics_dir + "metric_%s_%s_%s.npy" % (s, spacing, h)
        ...
        D_fname = metrics_dir + "metric_%s_%s_lrh.npy" % (s, spacing)
        if os.path.exists(D_fname):
            D_filtered = np.load(D_fname)
        elif len(hemis) == 2:
    ...
    it = (delayed(compute_metric)(s, ["lh"], spacing="ico3") for s in subjects)
    out = pll(it)
